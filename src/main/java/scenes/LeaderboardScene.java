...
public class LeaderboardScene {
  ...
  public void initialize() {
    ...
    scoreText4.setText("");
    scoreText5.setText("");
    combobox.getItems().addAll("highest score", "longest time survival", "enemies killed",
        "accumulate score");
  ...
  public void comboboxChange() {
    ...
    for (int i = 0; i < toPrintOut.size(); i++) {
      ...
      if (status == 1) {
        ...
        scoreText.get(i)
            .setText(String.valueOf(toPrintOut.get(i).getLongestTimeSurvivalInEndlessMode()));
      } else if (status == 3) {
